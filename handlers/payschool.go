...
// SmsPayload represents the SMS request payload
...
func SendSms(to, message string) error {
	log.Printf("[DEBUG] Sending SMS to %s with message: %s", to, message) // Debug log
	...
	if resp.StatusCode != http.StatusOK {
		log.Printf("[ERROR] Failed to send SMS, status: %d, response: %s", resp.StatusCode, body)
		return fmt.Errorf("failed to send SMS")
	}

	log.Println("[SUCCESS] SMS sent successfully!")
	return nil
}
...
//     if smsErr != nil {
//         log.Printf("Payment successful but failed to send SMS: %v", smsErr)
//     }
// }()

// Redirect to the parent page immediately, without waiting for the SMS response
// http.Redirect(w, r, "/parent?status=success", http.StatusSeeOther)
// return nil // Return nil since the payment was successful
...
// 			http.Redirect(w, r, "/parent?status=failed", http.StatusSeeOther)
// 		}
...
// 			http.Redirect(w, r, "/paren?status=failedt", http.StatusSeeOther)
// 		} else {
