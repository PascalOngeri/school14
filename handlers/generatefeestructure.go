...
func FeeReportHandler(w http.ResponseWriter, r *http.Request, db *sql.DB) {
    // Retrieve admission number from cookie
    admCookie, err := r.Cookie("adm")
    if err != nil {
        log.Printf("Error getting adm cookie: %v", err)
        http.Redirect(w, r, "/login", http.StatusSeeOther)
    ...
    // Query the database for the latest payment record
    var amount, bal float64
    err = db.QueryRow("SELECT amount, bal FROM payment WHERE adm = ? ORDER BY id DESC LIMIT 1", adm).Scan(&amount, &bal)
    if err != nil {
        log.Printf("Error fetching payment record: %v", err)
        http.Error(w, "Error fetching payment record", http.StatusInternalServerError)
        return
    }

    // Query for school name and logo icon
    var schoolName, icon string
    err = db.QueryRow("SELECT name, icon FROM api").Scan(&schoolName, &icon)
    if err != nil {
        log.Printf("Error fetching school name or icon: %v", err)
        http.Error(w, "Error fetching school name", http.StatusInternalServerError)
        return
    }
    ...
    // Add the school logo (check the icon path or URL)
    
    pdf.ImageOptions(icon, 80, 10, 50, 0, false, gofpdf.ImageOptions{ImageType: "PNG"}, 0, "")


    // Set font and add school name and fee structure heading
    pdf.SetFont("Arial", "B", 16)
    pdf.Ln(50)
    pdf.CellFormat(0, 10, schoolName, "", 1, "C", false, 0, "")
    pdf.CellFormat(0, 10, "Admission Number "+adm+" Fee Receipt", "", 1, "C", false, 0, "")
    pdf.Ln(10)

    // Admission Number Section
    pdf.SetFont("Arial", "", 12)
    pdf.CellFormat(40, 10, "Admission Number: ", "", 0, "L", false, 0, "")
    pdf.CellFormat(50, 10, adm, "", 1, "L", false, 0, "")
    pdf.Ln(5)

    // Table Header
    pdf.SetFont("Arial", "B", 12)
    pdf.CellFormat(40, 10, "Admission Number", "1", 0, "C", false, 0, "")
    pdf.CellFormat(75, 10, "Amount", "1", 0, "C", false, 0, "")
    pdf.CellFormat(75, 10, "Balance", "1", 1, "C", false, 0, "")

    // Table Data
    pdf.SetFont("Arial", "", 12)
    pdf.CellFormat(40, 10, adm, "1", 0, "C", false, 0, "")
    pdf.CellFormat(75, 10, fmt.Sprintf("%.2f", amount), "1", 0, "C", false, 0, "")
    pdf.CellFormat(75, 10, fmt.Sprintf("%.2f", bal), "1", 1, "C", false, 0, "")

    // Save and serve the PDF
    outputFileName := "latest_payment_report.pdf"
    err = pdf.OutputFileAndClose(outputFileName)
    if err != nil {
